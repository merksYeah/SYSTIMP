 ...
 * @author deathman28
 */
public class ViewUpdateDeliveryDate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            so.setOrderIds(request.getParameterValues("radio"));
            so.setOrder_id(Integer.parseInt(request.getParameter("radio")));
            ArrayList<Product> products = salesDB.getOrderDetailsByOrder(so);
            request.setAttribute("products",products);
            request.setAttribute("order",so);
            if(request.getParameter("inlineRadioOptions").equals("A")){
            rd = request.getRequestDispatcher("UpdateDeliveryDate.jsp");
            rd.forward(request,response);
            }
            else{
                so.setStatusCode("C");
                salesDB.updateSalesOrder(so);
                rd = request.getRequestDispatcher("/ToForReorder");
                rd.forward(request,response);
            }
