 ...
 * @author deathman28
 */
public class UpdateInsufficientOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            so.setOrderIds(request.getParameterValues("orderid"));
            so.setStatusCode(request.getParameter("inlineRadioOptions"));
            
            salesDB.updateSalesOrder(so);
            response.sendRedirect("/CHEMRES/ToInsufficientOrders");
        }
    }
